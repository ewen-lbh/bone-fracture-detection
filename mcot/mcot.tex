...
Il semble important de s'intérésser à l'automatisation du processus de détection et de classification de telles
blessures, afin de fluidifier l'orientation dans les services hospitaliers. En effet, les hopitaux sont ces derniers
temps souvent en sous-effectif, et fournir un premier diagnostic sans intervention professionnelle pourrait améliorer la
situation. 
...
conséquences parfois permanentes durant la suite de la vie du patient.  Plus particulièrement, la détection et
...
En 1986, John F. Canny met au point un algorithme de détection des bords\cite{canny}, qui permet de passer d’une image source à
une image noir et blanc, ou les pixels blancs. Cependant, l'os présente une porosité qui se manifeste sur les images
radiographiques par des irrégularités de contraste. Si l'algorithme de détection de bords est trop sensible aux
changements de contraste, il détectera des lignes parasites. Cette difficulté rend l'application directe, sans
pré-traitement ni post-traitement de l'algorithme de Canny à une image radiographique impertinente.
...
\begin{itemize}
    \item D'explorer la détection et classification sans apprentissage automatique, mais avec des techniques d'analyse
        d'image plus classiques (détection de bords, identification de segments de droites, calcul d'angles),
