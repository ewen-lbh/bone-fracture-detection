...
function CannyEdgeDetector()
    ...
    im = double(imsmooth(im,"Gaussian",sigma));
    %% figure; imshow(NormalizeMatrix(im));
    title('Gaussian Filter');
    if saveImage
        imwrite(NormalizeMatrix(im), '3_gaussian.jpg');
    end
    
    % Find the intensity gradient of the image
    Gx = SobelFilter(im, 'x');
    Gy = SobelFilter(im, 'y');
    Gx = imsmooth(Gx,"Gaussian",sigma);
    Gy = imsmooth(Gy,"Gaussian",sigma);
    %% figure; imshow(abs(NormalizeMatrix(Gx)));
    title('Gx Sobel Filter');
    if saveImage
        imwrite(abs(NormalizeMatrix(Gx)), '4_gx_sobel.jpg');
    end
    %% figure; imshow(abs(NormalizeMatrix(Gy)));
    title('Gy Sobel Filter');
    if saveImage
        imwrite(abs(NormalizeMatrix(Gy)), '5_gy_sobel.jpg');
    end
    ...
    title('Gmag');
    if saveImage
        imwrite(NormalizeMatrix(Gmag), '6_gmag.jpg');
    end
    ...
    if saveImage
        imwrite(Gmag, '7_non_maximum_suppression.jpg');
    end
    
    % Perform double thresholding
    %% highThreshold = max(max(Gmag))*highThresholdRatio;
    %% lowThreshold = highThreshold*lowThresholdRatio;
    fprintf("max Gmax is %f\n", max(max(Gmag)));
    ...
    if saveImage
        imwrite(Gmag, '8_double_threshold.jpg');
    end
    ...
    if saveImage
        imwrite(Gmag, '9_edge_tracking.jpg');
    end
    ...
    if saveImage
        imwrite(Gmag, '10_final.jpg');
    end
    
%     % MATLAB canny comparison
%     im = imread('Test_Photos/bone.jpeg');
%     im = rgb2gray(im);
%     im = edge(im, 'canny');
%% %     figure; imshow(im);
%     title('MATLAB');
